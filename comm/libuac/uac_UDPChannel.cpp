...
void UDPChannel::on_disconnected()
{
	...
	//只上报记录发送方
	if(CONNECTED==m_state && stun && m_send_win.ss.allsendB>m_recv_win.recv_sizeB)
	{
		...
		inf.sizeKB = (unsigned int)(m_send_win.ss.allsendB>>10);
		inf.sec = (GetTickCount()-m_begin_tick)/1000;
		inf.ttlMS = m_send_win.ttl/1000;
		inf.send_num = m_send_win.send_num;
		inf.resend_num = m_send_win.ss.resend_pks;
		inf.resend_timeo_num = m_send_win.ss.resend_timeo_pks;
		inf.other_rerecv_num = m_send_win.ss.other_rerecv_pks;
...
int UDPChannel::send(memblock *b,bool more/*=false*/)  //-1:false; 0:send ok; 1:put int sendlist
{
	...
	m_send_win.ss.max_seg_num = m_send_win.send_num;
	...
	{
		TimerSngl::instance()->register_timer(this,TIMER_SEND, g_uac_conf.sendtimer_ms);
		m_bsend_timer = true;
	}
...
int UDPChannel::send_to(char *buf,int len,ULONGLONG utick)
{
	...
	int ret = UDPChannelHandler::send_to(buf,len,utick);
	//test
	//ret = len;
	if (buf[UDPS_CONN_HEAD_LENGTH] == UDPS_CHANNEL_CMD_DATA)
	{
		if (ret == len)
		{
			...
			m_send_win.second_allsendB += len;
			m_send_win.ss.allsendB += len;
			m_send_win.ss.allsend_pks++;
			statisticsSngl::instance()->m_sendspeed.add(len);
		}
		else
		{
			m_send_win.ss.sendfaild_count++;
			statisticsSngl::instance()->on_sendfaild(errno);
		}
	}
	return ret;
}
...
int UDPChannel::handle_send(bool roolcall/*=false*/)
{
	...
	m_send_win.ss.setspeedB = m_send_win.spctrl.csp.speedB;
...
//		max_count=1000;
	...
	for(SendIter it=m_send_list.begin();it!=m_send_list.end()&&n<max_count;++it)
	{
		...
		{
			//正常发送不限窗
			if (pack->block->datasize != send_to(pack->block->buffer, pack->block->datasize, utick))
			{
				bsenderr = true;
				break;
			}
			statisticsSngl::instance()->m_valid_sendspeed.add(pack->block->datasize);
		...
		else if(pack->send_count<g_uac_conf.max_resend_num)
		{
			...
			if(pack->nak_count>1 && 1==pack->send_count)
			{
				...
				//timeout_us = 3*m_send_win.ttl + 100000;
			...
			else
			{
				...
				else
				{
					...
					if((pack->last_send_tick + timeout_us) < utick)
					{
						...
						bsend = true;
						m_send_win.ss.resend_timeo_pks++;
					}
				}
			}
			if(bsend)
			{
				...
				pack->nak_count=0;
				n++;rn++;
				m_send_win.ss.resend_pks++;
				m_send_win.ss.resendB += pack->block->datasize;
			}
		}
		else
		{
...
int UDPChannel::handle_recv_data(PTLStream& ps,memblock* b)
{
	...
	statisticsSngl::instance()->m_recvspeed.add(b->datasize);
	...
	{
		statisticsSngl::instance()->m_valid_recvspeed.add(b->datasize);
...
int UDPChannel::handle_recv_ack(PTLStream& ps)
{
	...
	{
		m_send_win.ss.other_rerecv_pks = ack.rerecv_num;
		if(ack.ack_sequence>m_send_win.ack_sequence)
		{
	...
	if(m_send_win.win_low_line<=low_line)
	{
		...
		m_send_win.ss.low_line = low_line;
		m_send_win.spctrl.other_recv_win_num = ack.recv_win_num;
		m_send_win.ss.other_wind_pks = ack.recv_win_num;
		m_send_list_max_size = m_send_win.spctrl.other_recv_win_num + m_send_win.spctrl.other_recv_win_num/2 + 20;
	}
				...
				else if((*it).send_num==(int)ack.seq_nums[i])
				{
					if(1==(*it).send_count)
					{
						...
						if(ttl<10000000)
						{
							...
							if(m_send_win.second_maxttl<ttl)
								m_send_win.second_maxttl = ttl;
