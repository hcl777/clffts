...
void sockpool::release_fd(UAC_SOCKET fd,int ithreadtoken)
{
	m_socks[fd].sock_state = SOCK_IDLE;
	MAXSOCK_CLR(fd,m_maxsock);
	m_socks[fd].close_state = 0;
	m_socks[fd].last_active_tick = GetTickCount();
	clear_memblock_list(m_socks[fd].sendlist, ithreadtoken);
	clear_memblock_list(m_socks[fd].recvlist, ithreadtoken);
...
int sockpool::recv(UAC_SOCKET fd,char* buf,int len)
{
	...
	else
	{
		...
		//限接收速度,如果延迟接收，则睡8毫秒
		int max_recvsize = statisticsSngl::instance()->get_max_app_recvsize();
		if(len > max_recvsize)
			len = max_recvsize; 
		if(max_recvsize==0)
			Sleep(1);
		...
		else
		{
			...
			statisticsSngl::instance()->m_app_recvspeed.add(recvsize);
		}
